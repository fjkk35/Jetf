mainApp.controller('EtlCustomerWorkLoadReportController', function ($scope, $http) {
    // 初始化資料
    $scope.customerList = [];
    $scope.groupList = [];
    ...
    // 表單資料
    $scope.searchData = {
        sDate: (function(){ var d = new Date(); d.setDate(d.getDate() - 1); return d; })(),
        eDate: new Date(),
        custTypeId: '1',
        selectedGroup: '',
        mainNumbers: ''
    ...
    $scope.loadGroupList = function () {
        $scope.loadingGroups = true;
        $http.get(Router.action('EtlCustomerWorkLoadReport', 'GetCustomerGroupList'))
    ...
    $scope.loadAllGroupDetails = function () {
        $http.get(Router.action('EtlCustomerWorkLoadReport', 'GetAllCustomerGroupDetails'))
            .then(function (response) {
    ...
    $scope.loadCustomerList = function () {
        $scope.loadingCustomers = true;
        $http.get(Router.action('EtlCustomerWorkLoadReport', 'GetCustomerList'))
            .then(function (response) {
                if (response.data && Array.isArray(response.data)) {
    ...
    $scope.getSelectedCustomers = function () {
        return $scope.customerList
            ...
            .map(function(customer) { return customer.Value; });
    };
    ...
    // 載入客戶格式列表
    $scope.loadCustomerTypeList = function () {
        $scope.customerTypeList = [
            { Text: '博豐', Value: '1' },
            { Text: '蝦皮', Value: '2' }
        ];
    ...
    // 解析主號(可多行)
    $scope.getMainNumberList = function () {
        if (!$scope.searchData.mainNumbers) return [];

        return $scope.searchData.mainNumbers
            .split(/\r?\n/)
    ...
    $scope.downloadExcel = function () {
        ...
        if (!$scope.searchData.custTypeId) {
            swal({
                title: "提醒",
                text: "請選擇客戶格式",
                icon: "warning"
        ...
        var postData = {
            ...
            custIds: selectedCustomers,
            custTypeId: $scope.searchData.custTypeId,
            mainNumbers: $scope.getMainNumberList()
        };

        $http({
            method: 'POST',
            url: Router.action('EtlCustomerWorkLoadReport', 'DownloadExcel'),
            data: postData,
