...
interface SeaClearanceDropdownOption {
    Value?: string | number;
    Text?: string;
    Id?: number;
    Name?: string;
...
interface SeaClearanceSearchForm {
    trackingNo: string;
    customerCode: string;
    type: string;
    postEntry: string;
    stepId: string;
    abnormalStateId: string;
    declNo: string;
    importer: string;
}

interface SeaClearanceSearchRequest {
    TrackingNo: string;
    DeclNo: string;
    CustCode: string;
    Type: string;
...
interface SeaClearanceListItem {
    Id: number;
    [key: string]: any;
}

interface SeaClearanceSearchResponse {
    Redirect?: boolean;
    error?: string;
    Data?: SeaClearanceListItem[];
    data?: SeaClearanceListItem[];
    TotalCount?: number;
}
...
interface SeaClearanceScope extends ng.IScope {
    data: SeaClearanceListItem[];
    loading: boolean;
    isSearched: boolean;
    currentPage: number;
    ...
    totalCount: number;
    totalPages: number;
    searchForm: SeaClearanceSearchForm;
    customers: SeaClearanceDropdownOption[];
    types: SeaClearanceDropdownOption[];
    postEntries: SeaClearanceDropdownOption[];
    steps: SeaClearanceDropdownOption[];
    abnormalStates: SeaClearanceDropdownOption[];
    recordsInfo?: string;
    [key: string]: any;
}

mainApp.controller('SeaClearanceController', ['$scope', '$http', function (
    $scope: SeaClearanceScope,
    $http: any
) {
    function createDefaultSearchForm(): SeaClearanceSearchForm {
        return {
            trackingNo: '',
            customerCode: '',
            type: '',
    ...
    function buildSearchRequest(): SeaClearanceSearchRequest {
        return {
    ...
    function getListResponseData<T>(response: { data: T[] | ApiResponse<T[]> }): T[] {
    ...
    $scope.loadCustomers = function () {
        $http.get(Router.action('SeaClearance', 'GetSeaCustomerList'))
            .then(function (response: { data: SeaClearanceDropdownOption[] }) {
    ...
    $scope.loadSteps = function () {
        $http.get(Router.action('Step', 'GetAllSteps'))
            .then(function (response: { data: SeaClearanceDropdownOption[] | ApiResponse<SeaClearanceDropdownOption[]> }) {
    ...
    $scope.loadData = function () {
        ...
        $http.post(Router.action('SeaClearance', 'SearchData'), buildSearchRequest())
            .then(function (response: { data: SeaClearanceSearchResponse }) {
    ...
    $scope.exportExcel = function () {
        ...
        $http.post(Router.action('SeaClearance', 'Excel'), buildSearchRequest())
            .then(function (response: { data: DownloadFileResponse }) {
                if (response.data.Redirect) {
                    openLoginPage();
                    return;
                }
    ...
    $scope.openDetail = function (id: number) {
