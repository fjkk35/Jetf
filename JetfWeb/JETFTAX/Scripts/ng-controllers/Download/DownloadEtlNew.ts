...
interface DownloadEtlNewResponse {
    Redirect?: boolean;
    status?: string;
    msg?: string;
    fileGuid?: string;
...
interface DownloadEtlNewScope extends ng.IScope {
    timeBetweenList: Array<{ value: string; text: string }>;
    form: {
        date: Date;
        timeBetween: string;
        sTime: string;
    ...
    loading: boolean;
    openDatePopup: () => void;
    onTimeBetweenChange: () => void;
    downloadAll: () => void;
}

mainApp.controller('DownloadEtlNewController', ['$scope', '$http', '$q', function (
    $scope: DownloadEtlNewScope,
    ...
    function openDownloadFile(response: DownloadEtlNewResponse): void {
    ...
    function createFormData(company: string): FormData {
        var form = document.getElementById('DownloadEtlNewForm') as HTMLFormElement;
    ...
    function postAction(actionName: string, company: string): ng.IPromise<DownloadEtlNewResponse> {
        return $http.post(Router.action('DownloadEtlNew', actionName), createFormData(company), {
            transformRequest: angular.identity,
            headers: { 'Content-Type': undefined }
        }).then(function (response: { data: DownloadEtlNewResponse }) {
            return response.data || {};
        });
    }

    function runDownloadTasks(index: number): ng.IPromise<void> {
        ...
        return postAction(task.action, task.company)
            .then(function (response: DownloadEtlNewResponse) {
