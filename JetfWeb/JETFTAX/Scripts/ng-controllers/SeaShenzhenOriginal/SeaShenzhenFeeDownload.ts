interface SeaShenzhenFeeDownloadResponse {
    fileGuid?: string;
    fileName?: string;
    msg?: string;
    Redirect?: boolean;
...
interface SeaShenzhenFeeDownloadScope extends ng.IScope {
    form: {
        dataDate: Date;
    };
...
mainApp.controller('SeaShenzhenFeeDownloadController', ['$scope', '$http', function (
    $scope: SeaShenzhenFeeDownloadScope,
    ...
    $scope.downloadExcel = function (): void {
        ...
        $http.post(Router.action('SeaShenzhenFeeDownload', 'ExportExcel'), {
            ...
            .then(function (response: ng.IHttpResponse<SeaShenzhenFeeDownloadResponse>): void {
