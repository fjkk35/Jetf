...
mainApp.directive('reconciliationCustomerSelector', ['$http', function ($http) {
        return {
            restrict: 'E',
            scope: {
                selectedCustomerMap: '=',
            ...
            link: function (scope, element) {
                ...
                function updateDisplay() {
                    ...
                    var names = {};
                    customersByType('SEA').concat(customersByType('AIR')).forEach(function (customer) {
                        if (!names[customer.CustCode]) {
                            names[customer.CustCode] = customer.CustName;
                        }
                    });
                    scope.displayText = '已選擇 ' + codes.length + ' 位客戶';
                    scope.displayFullText = codes.map(function (code) {
                ...
                scope.open = function () {
                    scope.keyword = '';
                    element.find('.reconciliation-customer-selector').modal('show');
                };
                scope.filteredCustomers = function (type) {
                    var keyword = (scope.keyword || '').trim().toLowerCase();
                    if (!keyword) {
                        return customersByType(type);
                    }
                    return customersByType(type).filter(function (customer) {
                        return (customer.CustCode || '').toLowerCase().indexOf(keyword) >= 0
                            || (customer.CustName || '').toLowerCase().indexOf(keyword) >= 0;
                    });
                };
                scope.groupsByType = function (type) {
                    return (scope.options.Groups || []).filter(function (group) {
                ...
                scope.selectAll = function (type) {
                    customersByType(type).forEach(function (customer) {
