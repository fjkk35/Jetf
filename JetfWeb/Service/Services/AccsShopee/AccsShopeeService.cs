...
namespace Service.Services.AccsShopee
{
    public class AccsShopeeService : _BaseService
    {
        ...
        public async Task<ResopnseModel> GetVerifyCodeImageAsync()
        {
            ...
            {
                return new ResopnseModel($"取得驗證碼失敗：{ex.Message}");
            }
        }
        ...
        public async Task<ResopnseModel> LoginAsync(AccsLoginRequest request)
        {
            try
            {
                using (var httpClient = GetHttpClient())
                {
                    ...
                    {
                        return new ResopnseModel("登入失敗：驗證碼錯誤或帳號密碼不正確");
                    }
                    ...
                    if (string.IsNullOrEmpty(token))
                    {
                        return new ResopnseModel("登入失敗：無法取得 Token");
                    }
            ...
            {
                return new ResopnseModel($"登入失敗：{ex.Message}");
            }
        }
        ...
        public async Task<ResopnseModel> QueryAsync(AccsQueryRequest request)
        {
            try
            {
                ...
                {
                    return new ResopnseModel("請輸入主提單號");
                }
                    ...
                    foreach (var mawb in mawbList)
                    {
                        try
                        {
                            ...
                            if (result.Any(r => r.Status == "SessionExpired"))
                            {
                                // 清除 Session Cookie
                                ClearSession();
                                return new ResopnseModel("您的登入已過期，請重新登入");
                            }
