...
namespace Service.Services.Step
{
    public class StepService : _BaseService
    {
        ...
        public ResopnseModel CreateStep(StepModel model)
        {
            ...
            {
                return new ResopnseModel("步驟名稱不能為空");
            }
            ...
            using (var transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        return new ResopnseModel($"步驟名稱「{model.StepName.Trim()}」已存在，請使用其他名稱");
                    }
        ...
        public ResopnseModel UpdateStep(StepModel model)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        transaction.Rollback();
                        return new ResopnseModel("找不到指定的步驟");
                    }
        ...
        public ResopnseModel DeleteStep(int id)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    if (detailCount > 0)
                    {
                        return new ResopnseModel("此步驟下還有步驟詳細資料，請先刪除所有步驟詳細後再刪除步驟");
                    }
        ...
        public ResopnseModel CreateStepDetail(StepDetailModel model)
        {
            ...
            {
                return new ResopnseModel("步驟詳細名稱不能為空");
            }

            if (model.StepId <= 0)
            {
                return new ResopnseModel("請選擇步驟");
            }
            ...
            using (var transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        return new ResopnseModel("指定的步驟不存在");
                    }

                    // 檢查步驟詳細名稱在同一步驟下是否重複
                    if (IsStepDetailNameExists(model.StepId, model.StepDetailName, null, transaction))
                    {
                        return new ResopnseModel($"在此步驟下，步驟詳細名稱「{model.StepDetailName.Trim()}」已存在，請使用其他名稱");
                    }
        ...
        public ResopnseModel UpdateStepDetail(StepDetailModel model)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        transaction.Rollback();
                        return new ResopnseModel("找不到指定的步驟詳細");
                    }
