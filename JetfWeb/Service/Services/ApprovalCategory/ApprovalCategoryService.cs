...
namespace Service.Services.ApprovalCategory
{
    public class ApprovalCategoryService : _BaseService
    {
        ...
        public ResopnseModel Create(ApprovalCategoryModel model)
        {
            ...
            {
                return new ResopnseModel("簽審類別名稱不能為空");
            }
            ...
            using (var transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        return new ResopnseModel($"簽審類別名稱「{model.CategoryName.Trim()}」已存在，請使用其他名稱");
                    }
        ...
        public ResopnseModel Update(ApprovalCategoryModel model)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        transaction.Rollback();
                        return new ResopnseModel("找不到指定的簽審類別");
                    }
        ...
        public ResopnseModel Delete(int id)
        {
            ...
            using (var transaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    if (usageCount > 0)
                    {
                        return new ResopnseModel("此簽審類別正在被使用中，無法刪除");
                    }
